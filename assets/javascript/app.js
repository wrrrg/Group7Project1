...
$(document).ready(function() {
  ...
  $("#use-gps-button").on("click", function() {
    $("#eventResults").prepend(
      $(
        '<div id="loading"> <img src="./assets/images/dribbble-songkick.gif" alt="songkick icon"></div>'
      )
    );
    useGPS = true;
...
var googleMaps = {
  ...
  success: function(position) {
    ...
    console.log(
      "The string for songkick is: " +
        userGPS.songKickStr +
        " and the string for Google is: " +
        userGPS.googleStr
    );
...
var songkick = {
  ...
  findCityGps: function(location) {
    ...
    $.ajax({
      ...
      var lat =
        response["resultsPage"]["results"]["location"][0]["metroArea"]["lat"];

      var long =
        response["resultsPage"]["results"]["location"][0]["metroArea"]["lng"];
  ...
  findEvents: function(location) {
    ...
    var queryURL =
      "https://api.songkick.com/api/3.0/events.json?apikey=" +
      apiKeySongKick +
      "&location=" +
      gps +
      "&per_page=15" +
...
appendEvents = function(eventResults) {
  ...
  for (var i = 0; i < eventResults.length; i++) {
    ...
    for (var j = 0; j < artistArr.length; j++) {
      ...
      var artistName =
        "<span class='artist-listing'><a href='#yourArtist' class='artist-click' >" +
        artist +
        "</a></span>";
    ...
    var eventName = $("<span class='name'>").html(
      "<br/>" + " " + name + " " + " "
    );

    // start time (military for now, might change with moment)
    var start = $("<span class='start'>").html(
      " Start time: " + eventStart + " " + "<br/>"
    );

    //  distance to the event
    var distance = $("<span class='distance'>").text(
      "Distance " + eventDist + " "
    );
    ...
    var address = $("<a class='address-link'>")
      .text(" " + address + " ")
      .attr({
        href: addressLink,
        target: "_blank"
    ...
    var songkickLink = $("<a class='more-info-link'>")
      .text("Tickets")
      .attr({
        href: link,
        target: "_blank"
